...
class EtfFilter:
    ...
    def __init__(self, etfs, tickers, start, end):
        ...
        self.df = yf.download(self.tickers + self.etfs, self.start, self.end)['Close'].dropna()
...
class PairsFilter2:
    ...
    def __init__(self, stock_type, sector="", number=1000, **kwargs):
        ...
        if stock_type != 'Stock':
            ...
            if number < len(diccionarioStocks[stock_type][sector]):
                ...
                print('El numero de tickers que escogiste es mayor al que tenemos, así que te muestro todos')
                self.tickers = diccionarioStocks[stock_type][sector]


        #tickers=diccionarioStocks[stock_type][sector]
        self.df = yf.download(self.tickers, **kwargs)['Close'].dropna()
        self.data_filtration = self.df.copy()
        self.pairs = []
        for i in range(len(self.tickers)):
            for j in range(i+1,len(self.tickers)):
